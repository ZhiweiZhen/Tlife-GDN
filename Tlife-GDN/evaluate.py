...
def get_final_err_scores(test_result, val_result):
    ...
    #order = np.sort(full_scores, axis=0)
    #all_scores = np.mean(order[:round(len(order)/4)],axis=0)
    return all_scores
...
def get_best_performance_data(total_err_scores, gt_labels, topk=1):
    ...
    #np.savetxt('predicted_result.txt',total_topk_err_scores)
    #np.savetxt('predicted_result.txt',pred_labels)
    #np.savetxt('val_result.txt',gt_labels)
    print(f'auc score: {auc_score}')
    return max(final_topk_fmeas), pre, rec, auc_score, thresold, total_topk_err_scores,final_topk_fmeas 
